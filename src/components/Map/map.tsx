...
import { toggleClustersLayer, toggleMapLayers, clusters, clusterCount, unclusteredPoint, addRasterLayer } from "./mapLayers";
...
function Map(props: MaplibreMapProps) {
  ...
  if (process.env.REACT_APP_API_KEY == null) {
    throw new Error("You have to configure env REACT_APP_API_KEY, see README");
     ...
     const riverPopUpRef = useRef(new maplibregl.Popup({ 
      offset: 15,
      closeButton: false, 
      closeOnClick: true,
      closeOnMove: true,
  ...
  const search = async (val: string) => {
    ...
    const res = await axios(
      `https://api.maphidro.info/api/stations?filters[name][$contains]=${val.toUpperCase()}`
    );
  ...
  const toggleSat = () => {
    ...
    if (satVisibility === "visible") {
      ...
      const clusterVisibility = map.getLayoutProperty(
        "cluster-count",
        "visibility"
      );
  ...
  useEffect(() => {
    ...
    mapLibre.on("load", function () {
      ...
      mapLibre.addLayer(
        clusters as LayerSpecification,
        "z-index-2"
      );

      mapLibre.addLayer(
        clusterCount as LayerSpecification,
        "z-index-2"
      );

      mapLibre.addLayer(
        unclusteredPoint as LayerSpecification,
        "z-index-2"
      );
      ...
      mapLibre.on("click", "clusters", function (e: { point: any; }) {
        ...
        const source: maplibregl.GeoJSONSource = mapLibre.getSource('stationIcons') as maplibregl.GeoJSONSource;
    ...
    mapLibre.on("click", "unclustered-point", function (e: any) {
      ...
      const stationObj: StationObject = {
  ...
  return (
    <div className="map-wrap">
      ...
      {spinner && (
        ...
        <Legends
          toggleLg={toggleSat}
          showLg={showSatToasts}
          position={"bottom-end"}
        />
        <AnimToast
          toggleAn={toggleSatMsg}
          showAn={showSatMsg}
...
<BottomNavigation
        ...
        >
          <BottomNavigationAction sx={{ color: grey[50] }}  onClick={toggleSearchToast} label="Search" icon={<ImageSearchIcon />} />
          <BottomNavigationAction sx={{ color: grey[50] }}  onClick={toggleSat} label="Satellite" icon={<SatelliteIcon />} />
          <BottomNavigationAction sx={{ color: grey[50] }} onClick={toggleClusters} label="Stations" icon={<RoomIcon />} />
        </BottomNavigation>
