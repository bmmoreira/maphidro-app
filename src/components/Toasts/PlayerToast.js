...
const PlayerToast = ({  position, showAc, toggleAc, mapref }) => {
  ...
  let cday = new Date();
  const [imgDate, setImgDate] = useState(cday.setDate(cday.getDate() - 7));
  ...
  const toggleRewind = () => {
    const satVisibility = mapref.getLayoutProperty("radar-layer", "visibility");
    if (satVisibility === "visible" && imgIndex > 0) {
      let currentIndex = imgIndex - 1;
      mapref.getSource("radar").updateImage({
        url: "/images/prec/overall/precp" + currentIndex + ".png",
      });
  ...
  const toggleForward = () => {
    const satVisibility = mapref.getLayoutProperty("radar-layer", "visibility");
    if (satVisibility === "visible" && imgIndex < 6) {
      let currentIndex = imgIndex + 1;
      mapref.getSource("radar").updateImage({
        url: "/images/prec/overall/precp" + currentIndex + ".png",
      });
        ...
        >
          <Toast show={showAc} onClose={toggleAc}>
            ...
            <Toast.Body>
              ...
              >
                <img
                  src="./images/buttons/b-control-rewind.png"
                  height="66"
                  width="61"
                  alt="Close"
                />
              ...
              >
                <img
                  src="./images/buttons/b-control-play.png"
                  height="66"
                  width="61"
                  alt="Close"
                />
              ...
              <br />
              Image date: {new Date(imgDate).getDate()}/
              {new Date(imgDate).getMonth() + 1}/
